...
public class CoinController {
    ...
    @GetMapping()
    public ResponseEntity<?> getCoins(@RequestParam(value = "minPrice") float minPrice, @RequestParam(value = "maxPrice") float maxPrice, @RequestParam(value = "isRising") boolean isRising, @RequestParam(value = "page") int page, @RequestParam(value = "pageSize") int pageSize, @RequestParam(value = "search") String search) {
        if (maxPrice == 0) maxPrice = Float.MAX_VALUE;

        return new ResponseEntity<>(coinService.getTopList(page, pageSize, minPrice, maxPrice, isRising, search),
                                    HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getCoinByTicker(@PathVariable String ticker, HttpServletRequest request) {
        ...
        return new ResponseEntity<>(coinService.getCoinByTicker(username, ticker), HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> setNote(@PathVariable String ticker, HttpServletRequest request, @RequestBody Map<String, String> requestBody) {
        String token = request.getHeader("Authorization").substring(7);
    ...
    public ResponseEntity<?> setFavorite(@PathVariable String ticker, HttpServletRequest request, @RequestBody Map<String, Boolean> requestBody) {
        String token = request.getHeader("Authorization").substring(7);
    ...
    @GetMapping("/favorite")
    public ResponseEntity<?> getFavoriteCoins(HttpServletRequest headers, @RequestParam int page, @RequestParam int pageSize) {
        String token = headers.getHeader("Authorization").substring(7);
        String username = jwtUtil.getUsernameFromJwtToken(token);
        return new ResponseEntity<>(coinService.getFavoriteCoins(page, pageSize, username), HttpStatus.OK);
    }
}
